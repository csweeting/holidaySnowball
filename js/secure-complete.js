...
(function($) {
	...
	*/
	function positionPages() {
		...
		if ( !reposition || $('#bcchf-step5').hasClass('js-bcchf-current-page') ) {
...
function executeMobileDFormPayPal() {
	
	ColdFusion.Ajax.submitForm('js-bcchf-donate-form', 'processDonation.cfc?method=executePayPalTransaction&returnFormat=JSON', mdonationResultHandlerPayPal, mdonationErrorHandler);
...
function mdonationResultHandlerPayPal(result) {
	...
	if (donationSuccessful == 0){
		...
		document.getElementById('exactResponseNegative').innerHTML='&nbsp;<br /><span class="registerError"><strong>Transaction not successful.<br />Message: '+responseMSG+'</strong></span><br />&nbsp;';
		...
		payPalCompleted = donationReturnProcess.CHARGEATTMPT.PPEXURL;
		
		window.location=payPalCompleted;
